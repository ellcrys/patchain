	...
	Convey("Object", t, func() {
		Convey(".Create", func() {
			Convey("Should initialize and create an object", func() {
				o := &tables.Object{Key: "some_key", Value: "some_value", PrevHash: util.UUID4(), PeerHash: util.UUID4()}
			...
			Convey("Should return unique constraint error if object with same prev_hash already exists", func() {
				o := &tables.Object{Key: util.RandString(5), Value: "some_value", PrevHash: util.RandString(10), PeerHash: util.RandString(10)}
				err := obj.Create(o)
				So(err, ShouldBeNil)

				o.ID = util.UUID4()
				o.PeerHash = util.RandString(10)
				err = obj.Create(o)
				var objs []tables.Object
				cdb.GetAll(&tables.Object{}, &objs)
				So(err, ShouldNotBeNil)
				So(err.Error(), ShouldContainSubstring, `violates unique constraint "idx_prev_hash"`)
			})
		...
		Convey(".RequiresRetry", func() {
			err := fmt.Errorf(`pq: duplicate key value (prev_hash)=('stuff') violates unique constraint "idx_prev_hash"`)
		...
		Convey(".CreatePartitions", func() {
			...
			Convey("Should successfully create initial partitions", func() {
				...
				Convey("Genesis pair must be chained; The first of the pair must be chained to the partition is attached to and the second must be chained to the first object", func() {
					for _, partition := range partitions {
						var all []tables.Object
						err := cdb.GetAll(&tables.Object{PartitionID: partition.ID, QueryParams: patchain.QueryParams{OrderBy: "timestamp asc"}}, &all)
						So(err, ShouldBeNil)
						...
						// The first of the pair must be chained to the partition is attached to
						So(all[0].PrevHash, ShouldEqual, util.Sha256(PartitionPrefix+partition.Hash))

						// second must be chained to the first object
						So(all[1].PrevHash, ShouldEqual, all[0].Hash)
					}
				})

				Convey("first partition prev hash must be equal to the SHA256 hash of it's  ID", func() {
		...
		Convey(".MustCreatePartitions", func() {
			...
			Convey("Should successfully create initial partitions", func() {
				...
				Convey("first partition prev hash must be equal to the SHA256 hash of its ID", func() {
					So(partitions[0].PrevHash, ShouldEqual, util.Sha256(partitions[0].ID))
				})
